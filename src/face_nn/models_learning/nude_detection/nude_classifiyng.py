...
def plot_accuracy(train_accs, val_accs, model_name):
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(f'training_results/plots/nude_classifier/{model_name}_accuracy_plot.png')
    plt.close()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                ...
                train_accs.append(epoch_acc.cpu().numpy() if isinstance(epoch_acc, torch.Tensor) else epoch_acc)
            else:
                val_losses.append(epoch_loss)
                val_accs.append(epoch_acc.cpu().numpy() if isinstance(epoch_acc, torch.Tensor) else epoch_acc)
...
def main():
    ...
    for model_name in model_names:
        ...
        trained_model, train_losses, val_losses, train_accs, val_accs = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=1, dataloaders=dataloaders, dataset_sizes=dataset_sizes, device=device)

        torch.save(trained_model.state_dict(), f'training_results/weights/nude_classifier/model_{model_name}.pth')
