...
class TripletNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(1280, 512), nn.ReLU(inplace=True), nn.Linear(512, 256)
        )
...
class TripletFaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        negative_img = random.choice(
            os.listdir(os.path.join(self.root_dir, negative_identity))
        )
...
def calculate_metrics(anchor_output, positive_output, negative_output, threshold):
    ...
    true_labels = torch.cat(
        [torch.ones_like(positive_distances), torch.zeros_like(negative_distances)]
    )
    predictions = torch.cat(
        [
            (positive_distances < threshold).float(),
            (negative_distances >= threshold).float(),
        ]
    )
...
def train_epoch_with_metrics(
    model, data_loader, criterion, optimizer, device, threshold
):
    model.train()
    running_loss = 0.0
    total_accuracy = 0.0
    ...
    for batch_idx, (anchor, positive, negative) in enumerate(
        tqdm(data_loader, desc="Training")
    ):
        ...
        optimizer.zero_grad()
        anchor_output, positive_output, negative_output = model(
            anchor, positive, negative
        )
        ...
        accuracy, f1 = calculate_metrics(
            anchor_output, positive_output, negative_output, threshold
        )
...
def validate_with_metrics(model, data_loader, criterion, device, threshold):
    ...
    avg_f1 = total_f1 / len(data_loader)
    print(
        f"Validation Loss: {avg_loss:.4f}, Accuracy: {avg_accuracy:.4f}, F1: {avg_f1:.4f}"
    )
...
if __name__ == "__main__":
    ...
    train_dataset = TripletFaceDataset(
        root_dir="data/face_simm/train", transform=transform
    )
