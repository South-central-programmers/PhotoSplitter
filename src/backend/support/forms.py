...
class AddUserPhotoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "identification_photo_1": forms.FileInput(
                attrs={"placeholder": "Добавьте файл"}
            ),
            "identification_photo_2": forms.FileInput(
                attrs={"placeholder": "Добавьте файл"}
            ),
        }
...
class AddPreviewPhotoForm(forms.ModelForm):
    class Meta:
        ...
        fields = ("headband",)
        widgets = {
            "headband": forms.ClearableFileInput(
                attrs={"placeholder": "Добавьте файл"}
            ),
        }
...
class ConfirmEventPasswordForm(forms.Form):
    password = forms.CharField(
        label="Пароль", widget=forms.TextInput(attrs={"placeholder": "Введите пароль"})
    )
